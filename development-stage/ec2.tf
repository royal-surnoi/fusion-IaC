...
resource "aws_vpc_security_group_ingress_rule" "allow_8080_ipv4" {
  security_group_id = aws_security_group.dev-stage-sg.id
  description       = [for description in var.ingress_rule : var.ingress_rule.description ]
  cidr_ipv4         = [for cidr_ipv4 in var.ingress_rule : var.ingress_rule.cidr_blocks ]
  from_port         = [for from_port in var.ingress_rule : var.ingress_rule.from_port ]
  ip_protocol       = [for ip_protocol in var.ingress_rule : var.ingress_rule.ip_protocol ]
  to_port           = [for to_port in var.ingress_rule : var.ingress_rule.to_port ]
}
